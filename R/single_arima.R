...
single_arima <- function() {
  ...
  ts_total <- stats::ts(zoo::coredata(monthly_total),
    start = c(
      as.numeric(format(start(monthly_total), "%Y")),
  ...
  ts_prop <- stats::ts(zoo::coredata(monthly_cov[, "prop_1ml"]),
    start = c(
      as.numeric(format(start(monthly_cov), "%Y")),
  ...
  ggplot2::ggplot() +
    geom_line(
      data = df_line %>% filter(.data$type == "Observed"),
      aes(x = date, y = .data$total_capacity), color = "black"
    ) +
    geom_line(
      data = df_line %>% filter(.data$type == "Forecast"),
    ...
    annotate("text",
      x = max(fc_dates), y = 1.05, label = annot_lbl,
